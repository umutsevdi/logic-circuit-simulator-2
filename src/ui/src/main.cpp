...
static void _err_cb(int error, const char* description)
{
    L_ERROR("GLFW Error %d: %s\n", error, description);
}
int main(int, char**)
{
    glfwSetErrorCallback(_err_cb);
    if (!glfwInit()) {
        return 1;
    }
    ...
    GLFWwindow* window = glfwCreateWindow(cfg.startup_win_x, cfg.startup_win_y,
        "Logic Circuit Simulator", nullptr, nullptr);
    if (window == nullptr) {
        return 1;
    }
