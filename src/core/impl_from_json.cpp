...
LCS_ERROR Point::from_json(const Json::Value& doc)
{
    x = doc["x"].isInt() ? doc["x"].asInt() : 0;
    y = doc["y"].isInt() ? doc["y"].asInt() : 0;
    return OK;
}
LCS_ERROR Node::from_json(const Json::Value& doc)
{
    if (doc["id"].isString()) {
...
LCS_ERROR Rel::from_json(const Json::Value& doc)
{
...
LCS_ERROR GateNode::from_json(const Json::Value& doc)
{
    if (doc["size"].isInt()) {
        size_t size = doc["size"].asInt();
        bool pass   = true;
        while (inputs.size() < size && pass) {
            pass = increment();
        }
    }
...
LCS_ERROR ComponentNode::from_json(const Json::Value& doc)
{
    if (!doc["use"].isString()) {
        return ERROR(Error::INVALID_COMPONENT);
    }
    return set_component(doc["use"].asString());
}
LCS_ERROR InputNode::from_json(const Json::Value& doc)
{
...
LCS_ERROR OutputNode::from_json(const Json::Value&) { return OK; }
LCS_ERROR ComponentContext::from_json(const Json::Value& doc)
{
...
LCS_ERROR Scene::from_json(const Json::Value& doc)
{
    ...
    if (doc["component"].isObject()) {
        ...
        err = component_context.value().from_json(doc["component"]);
        if (err) {
            return err;
        }
    }
