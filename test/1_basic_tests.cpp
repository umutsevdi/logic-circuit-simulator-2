...
TEST_CASE("Connect IN to OUT, Update")
{
    ...
    REQUIRE(s.get_node<Output>(o)->get() == state_t::FALSE);
...
TEST_CASE("[[v1,v2]->and, v2]->or->o")
{
    ...
    REQUIRE(s.get_node<Output>(o)->get() == state_t::TRUE);
    REQUIRE(s.get_node<Gate>(g_and)->get() == state_t::FALSE);
    REQUIRE(s.get_node<Gate>(g_or)->get() == state_t::TRUE);
}
...
TEST_CASE("Multiple Inputs with AND/OR Gates")
{
    ...
    auto g_and  = s.add_node<Gate>(gate_t::AND);
    auto g_or   = s.add_node<Gate>(gate_t::OR);
    auto g_nand = s.add_node<Gate>(gate_t::NAND);
    auto g_nor  = s.add_node<Gate>(gate_t::NOR);
    ...
    REQUIRE(s.get_node<Output>(o)->get() == state_t::TRUE);
    REQUIRE(s.get_node<Gate>(g_and)->get() == state_t::TRUE);
    REQUIRE(s.get_node<Gate>(g_or)->get() == state_t::TRUE);
    REQUIRE(s.get_node<Gate>(g_nand)->get() == state_t::TRUE);
    REQUIRE(s.get_node<Gate>(g_nor)->get() == state_t::FALSE);
}
...
TEST_CASE("1-Bit Adder Circuit")
{
    ...
    auto v1   = s.add_node<Input>();
    auto v2   = s.add_node<Input>();
    auto gate = s.add_node<Gate>(gate_t::OR);
    ...
    REQUIRE(s.get_node<Output>(sum)->get() == state_t::TRUE); // 1 + 0 = 1

    s.get_node<Input>(v1)->set(false);                        // v1 = 0
    s.get_node<Input>(v2)->set(true);                         // v2 = 1
    REQUIRE(s.get_node<Output>(sum)->get() == state_t::TRUE); // 0 + 1 = 1

    s.get_node<Input>(v1)->set(false);                         // v1 = 0
    s.get_node<Input>(v2)->set(false);                         // v2 = 0
    REQUIRE(s.get_node<Output>(sum)->get() == state_t::FALSE); // 0 + 0 = 0
}
...
TEST_CASE("Full Adder")
{
    ...
    REQUIRE(s.get_node<Output>(sum)->get() == state_t::TRUE);
    REQUIRE(s.get_node<Output>(c_out)->get() == state_t::FALSE);

    s.get_node<Input>(b)->set(true);
    REQUIRE(s.get_node<Output>(sum)->get() == state_t::FALSE);
    REQUIRE(s.get_node<Output>(c_out)->get() == state_t::TRUE);
